...
pltLaser = ax.scatter(x, y, z, c='red', label='Laser')
x, y, z = [np.array(fcd.points)[::ds, 0], np.array(fcd.points)[::ds, 1], np.array(fcd.points)[::ds, 2]]
pltFilament = ax.scatter(x, y, z, c='blue', label='Filament')
...
pltPiece = ax.scatter(x, y, z, c='black', label='Piece')
...
global pltTCP
pltTCP = ax.quiver(x, y, z, 3*u, 3*v, 3*w,  color='red')
# # Autoscale to adjust the axis range to fit the shown plot
ax.set_aspect('equal')
def init():
    pass
    return pltPiece,pltTCP
def update(frame):
    # stop process
    if frame == indexes.shape[0]:
        anim.event_source.stop()
    # Set segment to be analyzed
    env.process_index = indexes[frame]
    # Define rotation angle
    angle_change = angles[frame]
    if frame > 0:
        angle_change = angle_change - angles[frame - 1]
    # Step through process
    env.step(angle_change,'Y')
    # Plot results
    ...
    pltPiece._offsets3d = (x, y, z)  # Update only x and y positions
    x, y, z = [np.array(env.mod_TCP)[0, 3], np.array(env.mod_TCP)[1, 3], np.array(env.mod_TCP)[2, 3]]
    u, v, w = [np.dot(np.array(env.mod_TCP)[:3, :3], [0, 0, 1])[0],
               np.dot(np.array(env.mod_TCP)[:3, :3], [0, 0, 1])[1],
               np.dot(np.array(env.mod_TCP)[:3, :3], [0, 0, 1])[2]]
    ...
    pltTCP = ax.quiver(x, y, z, 3 * u, 3 * v, 3 * w, color='red')
    ax.set_aspect('equal')
    if frame == indexes.shape[0]-1:
        env.continue_process(2*(indexes[frame] - indexes[frame-1]))
    else:
        env.continue_process(indexes[frame+1] - indexes[frame])
    return pltPiece,pltTCP

anim = animation.FuncAnimation(figure, init_func=init, func=update, frames=indexes.shape[0], interval=500)  # number of frames, ms interval
